...
**Vulnerability:** A Hugging Face API Token (`HF_TOKEN`) was hardcoded in `script.js` directly, exposing it to anyone viewing the source code.
**Learning:** Client-side JavaScript cannot securely store secrets. Even if obfuscated or split into parts (`keyP1 + keyP2`), it is easily reconstructed by the browser and attackers.
**Prevention:** Use a "Bring Your Own Key" (BYOK) pattern for client-side apps where users provide their own keys (stored in `localStorage`), or proxy requests through a secure backend (e.g., Appwrite Functions) where secrets are stored as environment variables.
