        ...
        async function askMistral(promptText, isInteractive) {
            try {
                ...
                if (!hfToken) {
                    if (isInteractive) {
                        hfToken = window.prompt("Введите ваш токен Hugging Face для доступа к ИИ (HF_TOKEN):");
                        if (hfToken) {
                        ...
                        return null; // Gracefully abort if no token and not interactive
                    }
                }
                ...
                if (response.status === 401) {
                    ...
                    throw new Error("Unauthorized: Invalid HF_TOKEN. Token removed.");
                }
