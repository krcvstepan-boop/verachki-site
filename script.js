        ...
        // Expose helper to set token manually
        window.setAIToken = (token) => {
            localStorage.setItem("HF_TOKEN", token);
            console.log("Token saved.");
            showToast("AI Token сохранен");
        };
        ...
        async function askMistral(prompt, isInteractive = false) {
            try {
                ...
                if (!token) {
                    if (isInteractive) {
                        const input = window.prompt("Введите ваш Hugging Face Token (начинается с hf_):");
                        if (input && input.trim().startsWith("hf_")) {
                            token = input.trim();
                            localStorage.setItem("HF_TOKEN", token);
                        } else {
                            if (input) showToast("Неверный формат токена", "error");
                            return null;
                        }
                    } else {
                        console.warn("HF_TOKEN не найден. AI пропущен.");
                        return null;
                    }
                }
