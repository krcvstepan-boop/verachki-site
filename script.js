        ...
        // 🛡️ Sentinel: Removed hardcoded HF_TOKEN. API keys must not be exposed in client-side code.
        // Using BYOK (Bring Your Own Key) model via localStorage instead.
        ...
        async function askMistral(promptText, isInteractive) {
            try {
                ...
                if (!hfToken) {
                    if (isInteractive) {
                        hfToken = window.prompt("Введите ваш токен HuggingFace (HF_TOKEN) для использования ИИ:");
                        if (hfToken) {
                        ...
                        throw new Error("Требуется токен для фонового запроса");
                    }
                }
                ...
                if (response.status === 401) {
                    ...
                    throw new Error("Неверный токен (401). Токен удален.");
                }
