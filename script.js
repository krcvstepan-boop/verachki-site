        ...
        async function askMistral(prompt, isInteractive = false) {
            try {
                ...
                if (!hfToken) {
                    if (isInteractive) {
                        hfToken = window.prompt("Введите ваш Hugging Face Token для доступа к ИИ (токен сохраняется локально):");
                        if (!hfToken) return null;
                        localStorage.setItem('HF_TOKEN', hfToken);
                    } else {
                        return null; // Skip background calls without token
                    }
                }
                ...
                if (response.status === 401) {
                    ...
                    throw new Error("Unauthorized AI Access - Token cleared");
                }
