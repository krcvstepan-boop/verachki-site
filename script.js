        ...
        async function askMistral(prompt, isInteractive) {
            try {
                ...
                if (!hfToken && isInteractive) {
                    hfToken = window.prompt("Введите ваш Hugging Face Token (HF_TOKEN) для доступа к AI:");
                    if (hfToken) {
                    ...
                    return null; // Silent failure for non-interactive background triggers
                }
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        throw new Error("Unauthorized AI Token");
                    }
