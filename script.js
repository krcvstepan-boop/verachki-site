        ...
        async function askMistral(promptText, isInteractive) {
            try {
                ...
                if (!token) {
                    if (isInteractive) {
                        token = window.prompt("Введите ваш Hugging Face API Token для доступа к ИИ:");
                        if (token) {
                        ...
                        return null; // Don't prompt randomly in background
                    }
                }
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        throw new Error("Неверный токен. Токен сброшен.");
                    }
