        ...
        async function askMistral(promptText, isInteractive = false) {
            try {
                let hfToken = localStorage.getItem('HF_TOKEN');
                if (!hfToken) {
                    if (isInteractive) {
                        hfToken = window.prompt("Введите ваш Hugging Face API токен для использования AI:");
                        if (hfToken) {
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        throw new Error("Неверный токен Hugging Face (401 Unauthorized)");
                    }
