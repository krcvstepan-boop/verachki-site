        ...
        async function askMistral(promptText, isInteractive) {
            try {
                ...
                if (!hfToken) {
                    if (isInteractive) {
                        hfToken = window.prompt("Введите ваш HuggingFace Access Token для использования ИИ:");
                        if (hfToken) {
                            localStorage.setItem('HF_TOKEN', hfToken);
                        } else {
                            console.warn("HF_TOKEN не предоставлен. Отмена запроса к ИИ.");
                            return null;
                        }
                    } else {
                        return null; // Don't prompt randomly
                    }
                }
                ...
                if (response.status === 401) {
                    ...
                    throw new Error("Unauthorized: Неверный или истекший HF_TOKEN.");
                }
        ...
        async function tryTriggerAI(message) {
            ...
            if (isDirectCall || isRandomTrigger) {
                ...
                const promptText = isDirectCall ? message.replace(/^(ии|бот|система),/i, '').trim() : `Прокомментируй это сообщение: "${message}"`;
                const reply = await askMistral(promptText, isDirectCall);
