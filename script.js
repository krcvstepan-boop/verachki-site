        ...
        async function askMistral(prompt, isInteractive = false) {
            try {
                ...
                if (!token) {
                    if (isInteractive) {
                        token = window.prompt("ТРЕБУЕТСЯ КЛЮЧ ДОСТУПА (Hugging Face Token):");
                        if (token) {
                        ...
                        console.warn("AI token missing for background task.");
                        return null;
                    }
                }
                ...
                if (!response.ok) {
                    if (response.status === 401) {
                        ...
                        if (isInteractive) alert("Ошибка авторизации. Проверьте токен.");
                    }
