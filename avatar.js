...
class SoulAvatarSystem {
    constructor() {
        ...
        this.profilePetalMeshes = [];
    }

    init() {
        if (!this.canvas || !window.THREE || this.renderer) return;
        ...
        for (let i = 0; i < placeholders.length; i++) {
            this.observer.observe(placeholders[i]);
        }

        // Mutation tracking for new messages
        if (this.container) {
                    ...
                    mutation.addedNodes.forEach(node => {
                        if (node.nodeType === 1) {
                            if (node.classList.contains('soul-avatar-placeholder')) {
                                ...
                                const nested = node.getElementsByClassName('soul-avatar-placeholder');
                                for (let i = 0; i < nested.length; i++) {
                                    this.observer.observe(nested[i]);
                                }
                            }
                        }
                    });
    ...
    renderProfileAvatar(canvas, username, xp) {
        ...
        if (this.profileGroup) {
            ...
            this.profilePetalMeshes.forEach(mesh => {
        ...
        const { group, petalMeshes, speed, rotationAxis } = this.createAvatarGroup(username, xp);
        this.profileGroup = group;
        this.profilePetalMeshes = petalMeshes;
        ...
        const animate = () => {
             ...
             for (let i = 0; i < this.profilePetalMeshes.length; i++) {
                 this.profilePetalMeshes[i].material.uniforms.uTime.value = timeSeconds;
             }
    ...
    animate() {
        ...
        this.visibleAvatars.forEach(el => {
            ...
            const { group, petalMeshes, speed, rotationAxis } = this.getMesh(username);
            ...
            for (let i = 0; i < petalMeshes.length; i++) {
                petalMeshes[i].material.uniforms.uTime.value = timeSeconds;
            }
